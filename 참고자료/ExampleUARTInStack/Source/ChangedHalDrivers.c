...
PROVIDED ԁS ISԠWITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED,
...
static uint8 U0RxRing[60];
static uint8 U0_readidx;
static uint8 U0_writeidx;

static uint8 U1RxRing[60];
static uint8 U1_readidx;
static uint8 U1_writeidx;
...
void Hal_ProcessPoll ()
{
	...
	/* UART interrut process */
	if(U0_writeidx != U0_readidx)
	{
		// read and data process
		VOID osal_memset( U0RxRing, 0x00, 60 );
		U0_writeidx = 0;
		U0_readidx = 0;
	}
	
	if(U1_writeidx != U1_readidx )
	{
		// read and data process
		VOID osal_memset( U1RxRing, 0x00, 60 );
		U1_writeidx = 0;
		U1_readidx = 0;
	}
}
...
/**
 * UART0 RX Interrupt Vector
 */
#pragma vector=URX0_VECTOR
__near_func __interrupt void URX0_ISR(void)
{
	U0RxRing[U0_writeidx++] = U0DBUF;
        if( U0_writeidx == 60)
        {
                U0_writeidx = 0;
        }
...
/**
 * UART1 RX Interrupt Vector
 */

#pragma vector=URX1_VECTOR
__near_func __interrupt void URX1_ISR(void)
{
	U1RxRing[U1_writeidx++] = U1DBUF;
        if( U1_writeidx == 60)
        {
                U1_writeidx = 0;
        }
